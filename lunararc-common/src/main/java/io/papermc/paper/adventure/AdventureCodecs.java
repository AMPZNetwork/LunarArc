...
public final class AdventureCodecs {
    ...
    static final Codec<Object> PRIMITIVE_ARG_CODEC = ExtraCodecs.JAVA.validate(AdventureCodecs::filterAllowedArguments);

    /**
     * Real Paper's AdventureCodecs calls the private
     * {@code TranslatableContents.filterAllowedArguments(Object)} directly — real Paper compiles
     * INTO the same source tree as vanilla, so that private access is legal there. LunarArc is a
     * separate mod module, and cross-module access to it stayed blocked at runtime even with the
     * matching accesswidener entry (Architectury Loom's widener-to-AccessTransformer translation
     * for NeoForge did not take effect for this specific method — confirmed by the real
     * IllegalAccessError naming this exact method). Reimplemented directly instead of continuing
     * to depend on that translation working: this validates the same allowed-primitive-argument
     * set real Minecraft's {@code %s}-style translation strings accept (String and boxed number
     * types), which is stable, well-established behavior, not a guess at unrelated logic.
     */
    private static DataResult<Object> filterAllowedArguments(final Object arg) {
        if (arg instanceof String || arg instanceof Boolean || arg instanceof Byte
                || arg instanceof Short || arg instanceof Integer || arg instanceof Long
                || arg instanceof Float || arg instanceof Double) {
            return DataResult.success(arg);
        }
        return DataResult.error(() -> "Invalid argument type: " + arg);
    }
